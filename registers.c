...
void reg_assign(reg *r, char *bit_string, int free_flag)
{
  for (int i = 0; i < r -> size; i += 8) *(r -> vals + i/8) = (strtol(bit_string, 0, 2) >> i) & 0xff;
  if (free_flag) free(bit_string); //bad programming, the fuction that allocates the data should deallocate
}
...
{
  char *out = malloc(s); //need to modify function to take existing char*
